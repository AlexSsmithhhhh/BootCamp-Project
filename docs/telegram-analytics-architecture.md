...
- `admin_broadcast_sent` - админ отправил сообщение пользователям бота, включая `/post` wizard.
...
- `/post` - основной мастер отправки: пост всем сейчас, рассылка по сегментам или запланировать.
