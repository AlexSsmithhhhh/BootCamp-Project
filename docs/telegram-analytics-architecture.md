...
- `/new_post`, `/newpost` или `new post` - запускает мастер создания поста с выбором "сейчас" или "запланировать".
...
Мастер `/new_post` хранит промежуточное состояние админа в `admin_post_drafts`: выбранный режим, статус, время публикации и текущий `media_group_id`, если бот ожидает сбор альбома.
