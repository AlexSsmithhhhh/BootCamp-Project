...
- `/drop_post`, `drop post` или `дроп пост` - основной мастер создания поста с предпросмотром и подтверждением.
...
- `/post текст` - создает preview и публикует пост в `TELEGRAM_CHANNEL_ID` только после подтверждения.
- `/post` без текста - открывает мастер создания поста.
- `/post` ответом на фото/альбом/видео/PDF - создает preview медиа и публикует только после подтверждения.
...
- Фото/видео/PDF с caption `/broadcast текст` - сразу запускает медиа-рассылку.
...
Для медиа админ сначала отправляет материал боту в личку, затем отвечает на это сообщение командой. Поддерживаются одиночное фото, Telegram-альбом из нескольких фото, видео и PDF/document. Caption можно взять из исходного медиа или переопределить текстом команды. Caption самого медиа может начинаться с `/post текст` или `/broadcast текст`: `/post` всегда проходит через preview/confirm, а `/broadcast` сразу запускает рассылку.
...
Мастер `/drop_post` хранит промежуточное состояние админа в `admin_post_drafts`: выбранный режим, статус, время публикации, `payload` предпросмотра и текущий `media_group_id`, если бот ожидает сбор альбома. Перед публикацией или планированием админ подтверждает preview кнопкой.
