...
7. Admin confirms with `Подтвердить` or cancels with `Отменить`. Editing was intentionally removed from the preview step; to change a post, cancel and start `/post` again.
